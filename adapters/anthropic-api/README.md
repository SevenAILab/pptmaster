# PPTAgent · Anthropic API 适配器

## 安装

```bash
cd pptmaster
python -m pip install -r adapters/anthropic-api/requirements.txt
cp .env.example .env
```
...
python adapters/anthropic-api/example.py smallrig
...
本示例直接调用 Sub-Agent ④ `brand_positioning`。
