...
PSM模型也有其缺陷，没有考虑到价格变化随之的销量变化，也就是未考虑到整体市场容量的变化。同时，只是对于消费者意向的测试，但没有考虑到消费者真正的购买能力，也就是即使他们认为你处于合理的价格区间，但不一定买得起。最后，消费者对于价格的期待性具有一定的“欺骗性”，这跟调查者收入的高低相关，因为不同收入群体都某一价格区间的体感差异较大。所以在用PSM模型的时候，还需要加入其他因素进行交叉分析，从而不断趋近合理的定价区间。